...
func signIndex(t *testing.T, key ed25519.PrivateKey, index ReleaseIndex) ([]byte, []byte) {
	t.Helper()
...
func indexWithRecommended(versions []string, recommended ...bool) ReleaseIndex {
...
func TestMalformedRecommendationDegradesInsteadOfFailing(t *testing.T) {
	...
	policy := Policy{PublicKeyB64: base64.StdEncoding.EncodeToString(publicKey), LauncherVersion: "v2.0.0"}
	...
	t.Run("zero recommended is listed with a warning", func(t *testing.T) {
		...
		index, err := policy.VerifyReleaseIndex(payload, signature)
		if err != nil {
	...
	t.Run("a bad signature is still fatal", func(t *testing.T) {
		...
		if _, err := policy.VerifyReleaseIndex(payload, bogus); err == nil {
	...
	t.Run("an expired index is still fatal", func(t *testing.T) {
		...
		if _, err := policy.VerifyReleaseIndex(payload, signature); err == nil ||
...
func TestFilterReleasesForChannel(t *testing.T) {
	...
	t.Run("stable channel hides pre-releases", func(t *testing.T) {
		got := FilterForChannel(releases, false, "")
	...
	t.Run("pre-release channel shows everything", func(t *testing.T) {
		if got := FilterForChannel(releases, true, ""); len(got) != 4 {
	...
	t.Run("the installed pre-release stays visible on the stable channel", func(t *testing.T) {
		got := FilterForChannel(releases, false, "v2026.08.15-rc.9")
		found := false
...
func TestCompareReleasesNewestFirst(t *testing.T) {
	...
	slices.SortFunc(releases, CompareNewestFirst)
...
func TestRecommendedRelease(t *testing.T) {
	t.Run("returns the flagged release", func(t *testing.T) {
		...
		release, ok := Recommended(releases)
