...
func TestSignedManifestFromReleaseScriptVerifies(t *testing.T) {
	...
	policy := Policy{PublicKeyB64: pub}

	manifest, err := policy.VerifyManifest(manifestBytes, sigBytes, "v2026.08.05")
	if err != nil {
		t.Fatalf("launcher rejected the signed manifest: %v", err)
	}
	if err := VerifyFile(dir+"/ligand-x-runtime.zip", manifest); err != nil {
	...
	if _, err := policy.VerifyManifest(manifestBytes, sigBytes, "v2026.08.05"); err == nil {
